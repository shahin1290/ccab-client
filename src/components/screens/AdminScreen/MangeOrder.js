...
import {getAllOrders ,captureOrder, getOrder  } from '../../../redux/actions/orderAction';
...
export default function MangeOrder({match}) {
    ...
    /***********   Calling Reducer  ***************/
        ...
        const { loading :orderLoading , error : orderError ,orderList} = useSelector(
    ...
    /************* Functions *************/
        ...
        const getNotProcessedOrder =()=> orderList.filter((order)=>{
              return order.orderStatus ==='Not Processed'
          })
          
          const getVerifiedOrder =()=> orderList.filter((order)=>{
              return order.orderStatus ==='Processed' || order.orderStatus === 'Verified'
          })
          
          const getDeliveredOrder =()=> orderList.filter((order)=>{
            ...
            <div className="inner-container">
                        <div className="container-content">
                          {orderLoading ? (
                            ...
                            <h5>Total Incoming Orders : {getNotProcessedOrder().length }</h5>
                            ...
                            <Message variant="danger">{orderError}</Message>
                                ...
                                getNotProcessedOrder().length ? (
                                 getNotProcessedOrder().map((order, index) => (
                                    <tr key={order._id}>
                                      <td >{index + 1}</td>
                                      <td style={{width :'30%'}}>{order.course && order.course.name}</td>
                                      <td>{order.orderBy && order.orderBy.name}</td>
                                      <td>{getDate(order.createdAt)}</td>
                                      <td>{getPriceFormat(order.amount)+' '+order.currency}</td>
            ...
            <div className="inner-container">
                        <div className="container-content">
                          {orderLoading ? (
                            ...
                            <>
                            <h5>Total Incoming Orders : {getVerifiedOrder().length }</h5>
                            <table className="table">
                              <thead>
                                <tr>
                                  <th>#</th>
                                   ...
                                   getVerifiedOrder().map((order, index) => (
                                    <tr key={order._id}>
                                      ...
                                      <td>
                                        ...
                                        <button onClick={()=>{dispatch(captureOrder( order.course._id)) ;dispatch(getAllOrders())}} className="btn btn-primary btn-sm bg-success rounded-pill">Capture</button>
                                        : <span>Wating for Verifiation...</span>}</td>
                                    </tr>
                                  ))
                                ) : (
                                  <tr>
            ...
            <div className="inner-container">
                        <div className="container-content">
                          {orderLoading ? (
                            ...
                            <>
                            <h5>Total Incoming Orders : {getDeliveredOrder().length }</h5>
                            <table className="table">
                              <thead>
                                <tr>
                                  <th>#</th>
                            ...
                            <Message variant="danger">{orderError}</Message>
                                   ...
                                   getDeliveredOrder().map((order, index) => (
                                    <tr key={order._id}>
                                      ...
                                      <td>{getDate(order.createdAt)}</td>
                                      <td>{getPriceFormat(order.amount)+' '+order.currency}</td>
                                      <td>{order.orderStatus}<i class="ml-2 fas fa-check-circle text-success"></i></td>
                                    </tr>
                                  ))
                                ) : (
                                  <>

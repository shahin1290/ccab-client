...
import { getCourseDetails , updateCourse } from './../../../redux/actions/courseAction'
import { getUsers } from './../../../redux/actions/userAction'
import { Table, Col, Row, Modal, Button,Container , Form } from "react-bootstrap";
...
export default function UpdateCourese({match}) {
    ...
    const {course, loading :CourseDetailsloading , error : CourseDetailsError } = useSelector(
    ...
    useEffect(() => {
       // call the getter ( course Details  and users list )
        ...
        const _FilterUsers = (users, role)=>{
                    if ( role == 'MentorUser'){
                      return users.filter((user)=> (user.user_type == role) || (user.user_type == 'AdminUser'))  
                    }
        ...
        const _handleSelectStudent = ()=>{
           let item ;
            ...
            if ( students.length < Number(seats) )
                setStudents([ ...students , {name : student[0].name , _id:student[0]._id } ])
                else{
  ...
  const submitHandler = (e) => {
    ...
    if (titleWithAnswer.length){
      ...
      dispatch(updateCourse({ info_list : infoData,name :name, video_path:VideoUrl},course._id))
    }
  }
        ...
        <div className="edit-cource-section">
            <div className="auto-container">
                ...
                <div className="clearfix">
                    ...
                    <a href="/mentor-courses-list" className="see-all">Add Content</a>
                    </div>
                </div>
                </div>
                <div>
                    {error?<p className="text-danger bg-light p-2 ">{error}</p>:(
                        UpdateSuccess?<p className="text-success bg-light p-2 ">Course Updated successfully</p> :null
                    ) }
                ...
                <div className="row clearfix">
                    ...
                    <div className="inner-column">
                        ...
                        <form method="post" action="index.html">
                            ...
                            <input type="text" name="course-title" defaultValue placeholder="Course Title" value={name} required
                            ...
                            <textarea  name="message" placeholder="Shortly describe this course" 
                                ...
                                <div className="left-column col-lg-7 col-md-12 col-sm-12">
                                    ...
                                    <div className="edit-course-form">
                                        ...
                                        <div className="form-group col-lg-12 col-md-12 col-sm-12 text-center">
                                            ...
                                            >
                                            <span className="txt">
                                                Add List <i className="fa fa-angle-right" />
                                            </span>
                                            </div>
                                        </div>
                    ...
                    <div className="inner-column">
                        <h6>Video & Image</h6>
                        {/* Video Box */}
                        <div className="video-boxed" style={{backgroundImage: 'url(images/resource/video-image-3.jpg)'}}>   
                        <a onClick={()=>{handleOpenVideo()}} className="lightbox-image intro-video-box"><span className="fa fa-play"><i className="ripple" /></span></a>
                        </div>
                            {/* video Modal */}
                        ...
                        <input type="url" name="videoUrl" value={VideoUrl} placeholder="https://www.youtube.com/dummy-video.com" required
                        ...
                        <div className="border my-3"></div>
                            ...
                            {ImageLable?<img src={'https://server.ccab.tech/uploads/Bootcamp/'+ImageLable}/> :<p className="text-warning bg-light p-1">* No Image Uploaded</p>}
                            <span className="valid mb-3">Select (jpg / png )image </span>
                        <div className="input-group ">
                        
                            <input type="file" onChange={(e)=>{setImageLable(e.target.files[0].name)
                                 setImageUrl(e.target.files[0]);setUpdateErr(''); }}
                        ...
                        <div className="box-inner">
                            ...
                            <label> Mentor</label>
                            {!MentorsList.length&&<p className="text-warning bg-light p-1">* There is no Mentor Users</p>}
                            {/* <span className="select-category">Select a category</span> */}
                            <select className="custom-select-box px-2" onChange={(e)=>{ _handleSelectMentor(e.target.value.split(','))}}>
                                <option value="" disabled selected>Choose Mentor </option>
                                {MentorsList.length&&MentorsList.map((mentor)=>{
                                    return(
                                            <option value={[mentor._id,mentor.name]}>{mentor.name }{mentor.user_type=='AdminUser'&&' (Admin)'}</option>
                                            )
                                })}
                            </select>
                            ...
                            <div className="my-3">
                                    ...
                                    <span className="rounded-pill  px-2 py-1 m-2 bg-light">
                                        <i class="fas fa-plus-circle text-success"></i> {Mentor.name}
                                    </span>):
                                    (<p className="text-warning bg-light p-1">* Nothing Selected</p>)}
                            ...
                            <div className="form-group ">
                                ...
                                <label htmlFor="exampleDataList" className="form-label">Students</label>
                                {/* error message */}
                                {selectStudentErr&&<p className="text-danger bg-light p-1">{selectStudentErr}</p>}
                                 <input className="form-control bg-light" list="datalistOptions" id="exampleDataList" placeholder="search student..."
                                    onChange={(e)=>{setSelectStudentErr(''); setSelectedStudent(e.target.value); }} value={selectedStudent} />
 
                                <button type="button"  className="btn btn-success py-2 px-4 mt-2" onClick={_handleSelectStudent}>add</button>

                                <datalist id="datalistOptions"  >
                                    
                                    {StudentsList.length&&StudentsList.map((student)=>{
                                        return(
                                            <option data={student._id} value={student.name} key={student._id}
                                ...
                                <label className="mt-2">Selected Students : {students.length}/{StudentsList.length}</label>
                                <div className="my-3">

                                    {students.length?students.map((student)=>{
                                        return(
                                        <span className="rounded-pill  px-2 py-1  my-1 d-inline-block text-truncate bg-light">
                                            <a onClick={()=>{_handleUnselectStudent(student._id)}}
                                            ><i class="fas fa-minus-circle text-danger  cursor- pointer"></i></a> {student.name}
                                        </span>
                                        )
                                    }):<p className="text-warning bg-light p-1">* Nothing Selected</p>} 
                            ...
                            <div className="item-quantity">
                                <input className="quantity-spinner" type="number" defaultValue={price} 
                                name="quantity" onChange={(e)=>{setPrice(e.target.value)}}/>
                            </div>
                            </div>
                            ...
                            <div className="item-quantity">
                                <input className="quantity-spinner" type="number" min="0" defaultValue={seats} 
                                name="quantity" onChange={(e)=>{setSeats(e.target.value)}}/>
                            </div>
                            </div>
                        ...
                        <button type="button" className="theme-btn btn-style-one" style={{zIndex:"0"}}><span className="txt"
                        onClick={_handleupdateCourse}>Save Changes</span></button>
                        </div>
                    </div>
                    </div>
                </div>

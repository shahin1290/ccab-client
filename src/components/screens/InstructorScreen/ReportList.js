...
export default function SessionList() {
  ...
  const reportedSessions = () =>
    appointments &&
    appointments.filter((appointment) => appointment.status === "Reported");
              ...
              {appointments &&
                ...
                appointments.map((req) =>
                  req.sessions.map((session) => (
                    <div
                      key={req._id}
                      className='text-center-small-screen  mb-3'
                    >
                      <div className='sub-text font-weight-bold'>
                        {longEnUSFormatter.format(new Date(session.content))}{" "}
                      </div>
                      <Row className='pl-4 pr-5 mt-2'>
                        ...
                        <Col className='my-auto sub-text'>
                          {new Date(session.content).toLocaleTimeString(
                            undefined,
                            {
                              hour: "2-digit",
                              minute: "2-digit",
